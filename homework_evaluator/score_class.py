...
def extract_student_info_from_filename(filename):
    ...
    if len(parts) >= 6:
        # 班级是第三和第四部分的组合（parts[2] + '-' + parts[3]）
        # 例如：计科23 + '-' + 3班 = 计科23-3班
        class_name = parts[2] + '-' + parts[3]
        # 学号是倒数第二部分
        student_id = parts[-2]
        # 姓名是最后一部分，去掉文件扩展名
        name_part = parts[-1]
...
def compute_combined_similarity(text_similarity, image_similarity):
    """计算综合相似度"""
    global TEXT_WEIGHT, IMAGE_WEIGHT
...
def extract_student_answers(folder_path):
    ...
    for doc_file in doc_files:
        ...
        student_id, name, class_name = extract_student_info_from_filename(doc_file)
        ...
        if file_path.endswith('.docx'):
            ...
            try:
                ...
                pass
        results.append((student_id, name, class_name, content, images))

    return results

def tokenize(text):
    """使用jieba或简单方法进行分词，支持中英文"""
    global LANGUAGE
...
def compute_similarity_matrix(results):
    ...
    for student_id, name, class_name, content, images in results:
    ...
    for i in range(n):
        for j in range(i + 1, n):
            # 检查是否有空文档（没有文字）
            if len(documents[i]) == 0 or len(documents[j]) == 0:
                text_sim = 1.0
            else:
                # 计算文本相似度
...
def analyze_similarity(results, folder_path):
    ...
    print("正在计算平均相似度...")
    avg_similarities = []
    for i, (student_id, name, class_name, content, images) in enumerate(results):
        avg_sim = compute_average_similarity(similarity_matrix, i)
        avg_similarities.append((student_id, name, class_name, avg_sim))
        print(f"{name}: 平均相似度 = {avg_sim:.4f}")

    # 按平均相似度从低到高排序
    avg_similarities.sort(key=lambda x: x[3])

    # 打印排名
    print("\n" + "=" * 65)
    print("学生平均相似度排名（从低到高）")
    print("=" * 65)
    print(f"{'排名':<6} {'学号':<15} {'姓名':<10} {'班级':<10} {'平均相似度':<12} {'分数':<6}")
    print("-" * 65)

    # 计算分数
    min_sim = min(avg_similarities, key=lambda x: x[3])[3]
    max_sim = max(avg_similarities, key=lambda x: x[3])[3]

    student_scores = {}
    for rank, (student_id, name, class_name, avg_sim) in enumerate(avg_similarities, 1):
        score = compute_score_by_similarity(avg_sim, min_sim, max_sim)
        print(f"{rank:<6} {student_id:<15} {name:<10} {class_name:<10} {avg_sim:<12.4f} {score:<6}")
    ...
    for item in os.listdir(folder_path):
        if item.endswith('.txt'):
            # 尝试从文件名中提取章节名
            match = re.search(r'ch(\d+)', item.lower())
            if match:
                chapter_name = f"ch{match.group(1)}"
                break
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        ...
        f.write("学生平均相似度排名\n")
        f.write("=" * 65 + "\n")
        f.write(f"{'排名':<6} {'学号':<15} {'姓名':<10} {'班级':<10} {'平均相似度':<12} {'分数':<6}\n")
        f.write("-" * 65 + "\n")
        for rank, (student_id, name, class_name, avg_sim) in enumerate(avg_similarities, 1):
            score = compute_score_by_similarity(avg_sim, min_sim, max_sim)
            f.write(f"{rank:<6} {student_id:<15} {name:<10} {class_name:<10} {avg_sim:<12.4f} {score:<6}\n")
        f.write("=" * 65 + "\n")
...
if __name__ == "__main__":
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='学生作业相似度评估系统')
    parser.add_argument('folder_path', help='包含学生作业的文件夹路径')
    parser.add_argument('--language', '-l', default='chinese',
                       choices=['chinese', 'english'],
                       help='作业语言 (chinese 或 english，默认: chinese)')
    parser.add_argument('--evaluation', '-e', default='text',
                       choices=['all', 'text'],
                       help='评估方式 (all: 全部, text: 仅文本，默认: text)')
    ...
    print(f"当前语言模式: {'中文' if LANGUAGE == 'chinese' else '英文'}")
    print(f"当前评估方式: {'仅评估文本' if EVALUATION == 'text' else '评估全部（文本+图片）'}")
    print(f"文本权重: {TEXT_WEIGHT}, 图片权重: {IMAGE_WEIGHT}")
    print(f"DEBUG: 评估方式参数值: {args.evaluation}")
    print(f"DEBUG: 全局变量TEXT_WEIGHT: {TEXT_WEIGHT}")
    print(f"DEBUG: 全局变量IMAGE_WEIGHT: {IMAGE_WEIGHT}")
    sys.stdout.flush()

    results = extract_student_answers(args.folder_path)

    if results:
        analyze_similarity(results, args.folder_path)
